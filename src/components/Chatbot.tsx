...
type ChatStep =
  ...
  | "valor_plano"
  | "dificuldade"
  | "numero_cnpj" // Movido para penúltima posição
  | "finalizado";
...
const initialState: ChatState = {
  ...
  messages: [
    {
      type: "bot",
      text: "👋 Olá! Sou o Davi, assistente virtual da Bradesco Seguros. Vou te ajudar a encontrar o melhor plano PME para sua empresa!",
    },
  ],
  isTyping: false,
};
...
export default function Chatbot({ onClose }: ChatbotProps) {
  ...
  const validateInput = (step: ChatStep, value: string): boolean => {
    switch (step) {
      ...
      case "numero_cnpj": {
        // CNPJ agora é opcional - sempre retorna true
        return true;
      }
      case "valor_plano":
        return /\d+/.test(value);
  ...
  const handleSubmit = async (e: React.FormEvent | null, optionValue?: string) => {
    ...
    if (!value.trim() && step !== "numero_cnpj") return; // CNPJ pode estar vazio
    ...
    dispatch({ type: "ADD_MESSAGE", payload: { type: "user", text: value || "Pular esta etapa" } });    switch (step) {
    ...
    try {
      ...
      if (step === "whatsapp" && newData.nome && newData.whatsapp) {
        ...
        const id = await createLead({
          ...
          whatsapp: newData.whatsapp,
          email: whatsappEmail,
          temCnpj: false, // Inicialmente false, será atualizado depois se fornecido
        });
        ...
        console.log("Atualizando lead:", leadId, "com dados:", {
          ...
          status: nextStep === "finalizado" ? "completo" : "em_andamento", // Corrigido: verifica se o próximo step é finalizado
        });
    ...
    if (step === "numero_cnpj") {
      dispatch({ type: "ADD_MESSAGE", payload: { type: "user", text: "Pular esta etapa" } });
      
      const newData = { ...chatData };
      newData.numeroCnpj = ""; // CNPJ vazio ao pular
      
      const nextStep = getNextStep(step, newData);
      dispatch({ type: "PROCEED_STEP", payload: { nextStep, newData } });
      
      // Atualizar o lead quando pular o CNPJ (antes de finalizar)
      if (leadId && nextStep === "finalizado") {
        // Fazemos isso de forma assíncrona sem bloquear a UI
        void (async () => {
          try {
            await updateLead({
              leadId,
            ...
            // Enviar email
            console.log("Enviando email após pular CNPJ para leadId:", leadId);
            toast.loading("Enviando suas informações...");
            ...
            console.error("Erro ao finalizar processo após pular CNPJ:", error);
            toast.error("❌ Seus dados foram salvos, mas ocorreu um erro ao processá-los. Nossa equipe entrará em contato em breve!");
          }
        })();
      }
