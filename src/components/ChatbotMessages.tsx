    ...
    <div className="flex-1 overflow-y-auto p-2 sm:p-4 space-y-4 bg-gray-50">
      {messages.map((message, index) => (
        ...
        >
          <div className={`max-w-[85%] ${message.type === "user" ? "" : "flex items-start space-x-2"}`}>
            {message.type === "bot" && (              <div className="w-8 h-8 bg-bradesco-red rounded-full flex items-center justify-center flex-shrink-0 mt-1">
                <span className="text-white text-sm">{getStepIcon(step)}</span>
              </div>
            )}
            <div
              className={`p-3 rounded-2xl ${                message.type === "user"
                  ? "bg-bradesco-red text-white rounded-br-md"
                ...
                <div className="mt-3 space-y-2">
                  {message.options.map((option, optIndex) => (
                    <button
                      ...
                      className="block w-full text-left px-3 py-2 text-sm bg-gray-50 hover:bg-bradesco-red hover:text-white transition-colors rounded-lg border border-gray-200"
                    >
                      {option}
                    </button>
                  ))}
      ...
      {isTyping && (
        <div className="flex justify-start">
          <div className="flex items-start space-x-2">            <div className="w-8 h-8 bg-bradesco-red rounded-full flex items-center justify-center">
